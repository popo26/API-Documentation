...
export default function populateParams(count, parameters, form, path, httpMethod) {
  if (parameters) {
...
// export default function populateParams(count, parameters, form, path) {
//   if (parameters) {
//     for (let key in parameters) {
//       count++;
//       const node = document.createElement("input");
...
//       node.classList.add(`p${path}`);
//       node.classList.add(`${path}${count}`);
//       node.classList.add(`${parameters[key].in}`);
